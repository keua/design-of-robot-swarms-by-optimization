...
plot_all_series_development <- function() {
  ...
  load_experiment_series_development <- function(series, controller_type, scenario) {
    # load the ten runs
    for (i in 1:EXP_COUNT) {
      exp_name <- paste(controller_type, scenario, series, i, sep="_")
      print(exp_name)
  ...
  plot_controller_development <- function(series, controller_type) {
    # Aggregation
    agg_results <- load_experiment_series_development(series, controller_type, "agg")
    title_agg <- paste("Aggregation ", controller_type, " (", series, ")",  sep="")
    boxplot(agg_results, main=title_agg, ylab="Score", xlab="Iteration")
    # Foraging
    for_results <- load_experiment_series_development(series, controller_type, "for")
    title_for <- paste("Foraging ", controller_type, " (", series, ")",  sep="")
...
plot_single_experiment_development <- function(series, controller_type, scenario, i) {
