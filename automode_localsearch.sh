...
mpiexec -n 2 python3 -m mpi4py /home/jkuckling/AutoMoDe-LocalSearch/src/main.py -c ${config} -r $TMPDIR -s ${scenario} -i ${initial} -exe ${executable} -j ${job_name} -t ${controller_type} &>> $TMPDIR/output_${job_name}.txt
RET=$?
mv $TMPDIR/* $RESULTDIR
cd $JOBDIR
